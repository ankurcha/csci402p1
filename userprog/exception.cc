...
void Exit_Syscall(int status) {
    ...
    processTableLock->Acquire();
    if (processTable->processCounter == 1 && currentThread->space->childThreads
            == 0) {
...
void DestroyLock_Syscall(LockId id) {
    locksTableLock->Acquire();
    LockWrapper *targetLock =
            (LockWrapper*) currentThread->space->locksTable.Get(id);
    if (targetLock) {
        if (targetLock->mark && targetLock->counter == 0) {
            ...
            DEBUG(
                    'a',
                    "%s: DestroyLock_Syscall: Successfully deleted lock %d .\n",
                    currentThread->getName(), id);
        } else {
            //Cannot delete need to persist just mark for deletion
            targetLock->mark = true;
            DEBUG('a',
                    "%s: DestroyLock_Syscall: marked lock %d for deletion.\n",
                    currentThread->getName(), id);
        }
        ...
        DEBUG(
                'a',
                "%s: DestroyLock_Syscall: Unable to find lock %d for deletion.\n",
                currentThread->getName(), id);
    }
...
void AcquireLock_Syscall(LockId lockId) {
    locksTableLock->Acquire();
    LockWrapper *targetLock =
            (LockWrapper*) currentThread->space->locksTable.Get(lockId);
    if (targetLock == NULL) {
        DEBUG(
                'a',
                "%s: AcquireLock_Syscall: Unable to find lock %d for acquire.\n",
...
void DestroyCondition_Syscall(CVId id) {
    ...
    CVTableLock->Acquire();
    ConditionWrapper *target =
            (ConditionWrapper*) currentThread->space->CVTable.Get(id);
    if (target) {
        if (target->counter == 0) {
            //We can delete
            delete target->cv;
            ...
            DEBUG(
                    'a',
                    "%s : DestroyCondition_Syscall: Successfully deleted CV %d .\n",
                    currentThread->getName(), id);
            ...
            DEBUG(
                    'a',
                    "%s: DestroyCondition_Syscall: marked CV %d for deletion.\n",
                    currentThread->getName(), id);
        }
    } else {
        DEBUG(
                'a',
                "%s: DestroyCondition_Syscall: Unable to find CV %d for deletion.\n",
                currentThread->getName(), id);
    }
    CVTableLock->Release();
}
...
void WaitCV_Syscall(CVId cvId, LockId lockId) {
    ...
    LockWrapper *ConditionLockWrapper =
            (LockWrapper*) currentThread->space->locksTable.Get(lockId);
    ConditionWrapper *CV =
            (ConditionWrapper*) currentThread->space->CVTable.Get(cvId);
 ...
 */
int Receive_Syscall(int receiveMbox, int senderIDvaddr, int senderMboxvaddr,
        int vaddr) {
#ifdef NETWORK
    int bytesRead = -1;
    int senderID = -1;
...
unsigned int GetTimestamp_Syscall() {
    static long long starting_TS = -1;
    struct timeval tv;
    struct timezone tz;
    struct tm *tm;
    ...
    long long myTimestamp = ((long long)(tv.tv_usec + tv.tv_sec*1000000)); 
    ...
    if (which == SyscallException) {
        switch (type) {
            ...
            case SC_Create:
                ...
                Create_Syscall(machine->ReadRegister(4), machine->ReadRegister(
                        5));
                break;
            case SC_Open:
