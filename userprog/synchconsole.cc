...
static void SynchReadAvail(SynchConsole *SC) {
    SC->ReadAvail();
}
static void SynchWriteDone(SynchConsole *SC) {
    SC->WriteDone();
}
...
SynchConsole::SynchConsole(char *readFile, char *writeFile) {
    ...
    console = new Console(readFile, writeFile,
            (VoidFunctionPtr) SynchReadAvail, (VoidFunctionPtr) SynchWriteDone,
            (int) this);
}
...
void SynchConsole::WriteLine(char *line, int size) {
...
int SynchConsole::ReadLine(char *line, int size) {
    char c = 0;
    int num_read = 0;
    inputMutex->Acquire();
    while (c != '\n' && num_read < size) {
