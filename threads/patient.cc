...
void patients(int ID) {
    ...
    printf(
            "P_%d Got woken up, get out of line and going to counter for token\n",
            ID);
    receptionists[shortestline].peopleInLine--;
    ...
    if (test2active == true) {
        printf(
                "P_%d :TEST2: Join line and Waiting for doorboy to tell me to go\n",
                ID);
    } else
    ...
    if (test4active == false) {
        printf("P_%d: Found shortest pharmacy clerk line CL_%d len: %d\n", ID,
                shortestclerkline, length);
    } else {
        printf(
                "P_%d:TEST4: Found shortest pharmacy clerk line CL_%d len: %d\n",
                ID, shortestclerkline, length);
    }

    //wait in line for my turn
    clerks[shortestclerkline].patientsInLine++;
    cout << "P_" << ID << ": Waiting in line for clerk CL_"
            << shortestclerkline << " to attend to me, Line length: "
            << clerks[shortestclerkline].patientsInLine << "\n";

    clerks[shortestclerkline].ClerkCV->Wait(ClerkLinesLock);

    cout << "P_" << ID
            << " Got woken up, got out of line and going to the Pharmacy "
    ...
    clerks[shortestclerkline].ClerkTransCV-> Signal(
            clerks[shortestclerkline].ClerkTransLock);
    clerks[shortestclerkline].ClerkTransCV-> Wait(
            clerks[shortestclerkline].ClerkTransLock);
