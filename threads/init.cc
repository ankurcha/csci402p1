...
void doorboy(int ID) {
    ...
    while (true) {
        ...
        printf("DB_%d: Found %d patients waiting in line for D_%d\n", ID,
                doctors[myDoctor].peopleInLine, myDoctor);
    ...
    while (true) {
        ...
        while (doorboyLineLength <= 0) {
            if (waitingtime % 100 == 0) {
                cout << "D_" << ID
                        << ": Doctor could not find a doorboy waittime: "
                        << waitingtime << endl;
            }
        ...
        if (test7active == true) {
            int numYields = 35;
            cout << "D_" << ID << " :TEST7: Going on break for " << numYields
                    << " cycles!\n";
            ...
            cout << "D_" << ID << ": Going on break for " << numYields
                    << " cycles!\n";
            for (int i = 0; i < numYields; ++i) {
                currentThread->Yield();
            }
        ...
        if (test7active == true) {
            cout << "D_" << ID
                    << " :TEST7: Back from Break,Signalling patient to come in.\n";
        } else
            cout << "D_" << ID
                    << ": Back from Break,Signalling patient to come in.\n";
...
void cashier(int ID) {
    ...
    while (true) {
        ...
        if (cashiers[ID].lineLength > 0) { // someone in line
            ...
            if (test_state == 11)
                cout << "T11: ";
            cout << "Cash_" << ID << ": No one in line... going on break"
                    << endl;
...
void clerk(int ID) {
    while (true) {
        ...
        clerks[ID].ClerkTransCV->Wait(clerks[ID].ClerkTransLock);
        //Collect payment
        cout << "C_" << ID << ": The cost for the medicines are:"
                << clerks[ID].fee << " dollars" << endl;
        // add this payment to our total collected
        PaymentLock->Acquire();
        ...
        for (int i = 0; i < numClerks; i++) {//Check for waiting patients
            if (clerks[i].patientsInLine > 0) {
                cout << "H_" << ID << ": found CL_" << i << " sleeping and "
                        << clerks[i].patientsInLine
                        << " waiting -> Signaling Clerk\n";
...
void HospINIT(int testmode = 0) {
    ...
    if (testmode != 1 && testmode != 51 && testmode != 52 && testmode != 53) {
        ...
        //3. Cashiers
        numCashiers
                = (Random() % (MAX_CASHIER - MIN_CASHIER + 1) + MIN_CASHIER);
        ...
        //7. Patients
        numPatients = Random() % (MAX_PATIENTS - MIN_PATIENTS + 1)
                + MIN_PATIENTS;
        hospitalLock->Acquire();
